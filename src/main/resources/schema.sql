...
-- Migração: adiciona coluna primeiro_acesso se ainda não existir (bancos já existentes)
-- Feita via DataInitializer (Java) para compatibilidade com MySQL padrão
...
-- ── 3. TIPO DE SERVIÇO (tabela de domínio) ────────────────
...
-- ── 4. TIPO DE DESPESA (tabela de domínio) ────────────────
...
-- ── 5. CIDADE (tabela de domínio — regional SJC) ──────────
...
-- ── 6. TIPO DE COMBUSTÍVEL (tabela de domínio) ────────────
...
-- ── 7. ORDEM DE SERVIÇO ← tabela central ─────────────────
CREATE TABLE IF NOT EXISTS ordem_servico (
                                             id              INT      NOT NULL AUTO_INCREMENT,
                                             viatura_id      INT      NOT NULL,
                                             usuario_id      INT      NOT NULL,
                                             tipo_servico_id INT      NOT NULL,               -- FK tipo_servico
                                             cidade_id       INT      NULL,                   -- FK cidade (selecionada na abertura)
                                             tipo_despesa_id INT      NULL,                   -- FK tipo_despesa (opcional)
    ...
    numero_os_externo    VARCHAR(20) NULL,   -- nº da OS no SGI (opcional)
    created_at           DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
...
-- ── 7. ABASTECIMENTO ← filho da OS (os_id nullable) ───────
CREATE TABLE IF NOT EXISTS abastecimento (
                                             id                 INT           NOT NULL AUTO_INCREMENT,
                                             os_id              INT           NULL,            -- NULL = abastecimento avulso (sem OS)
                                             viatura_id         INT           NOT NULL,
                                             usuario_id         INT           NOT NULL,
                                             tipo_combustivel   VARCHAR(20)   NOT NULL,        -- gasolina|etanol|diesel|gnv
...
-- ── 9. MANUTENÇÃO ← extra-OS, direto na viatura ──────────
